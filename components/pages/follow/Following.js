...
import {  Image, Text, View, StyleSheet,  FlatList, ImageBackground, RefreshControl ,ScrollView, TouchableHighlight } from 'react-native';
...
class FollowingScreen extends React.Component {
  ...
  fetchData = async () => {
    fetch(`http://68.183.209.228:3000/api/follows/5c2b734a4290ea0e3b586751/following/1`)
    .then(response => response.text())
      .then(JSON.parse)
      .then(accounts => this.setState(state => ({
          accounts: [...state.accounts , ...accounts.following],
          loading: false
      })))
  ...
  renderContactsOnApp = ({ item, index }) => {
    let imageUrl = item.followee.profileImage;
    let userId = item.followee.id; 
    return (
      <View style={styles.container}>
        ...
        <TouchableHighlight onPress={() => {
            ...
            <Text style={styles.paragraph}>
              {item.followee.firstName} {item.followee.lastName}
            </Text>
        </TouchableHighlight>
